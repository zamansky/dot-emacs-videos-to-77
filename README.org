...
- live-py
  ...
  - class addTest(TestCase)  -- test_name -- self.assertEqual(a,b)
  - C-c M-d
- cloud-theme
- general.el
- focus mode
...
(add-to-list 'load-path (expand-file-name "~/Sync/shared/aweshell"))
...
(define-key z-map (kbd "e") 'aweshell-dedicated-toggle)
